...
function taumBday(b, w, bc, wc, z) {
    ...
    // console.log(BigInt(b * bc + w * wc))
    if (bc === wc) {
        return BigInt(b) * BigInt(bc) + BigInt(w) * BigInt(wc)
    } else if (wc > (bc + z)) {
        const buy = BigInt(w) + BigInt(b)
        return  BigInt(buy) * BigInt(bc) + BigInt(w) * BigInt(z)
    } else if (bc > (wc + z)) {
        const buy = BigInt(b) + BigInt(w)
        return BigInt(buy) * BigInt(wc) + BigInt(b) * BigInt(z)
    }
    return BigInt(b) * BigInt(bc) + BigInt(w) * BigInt(wc)
